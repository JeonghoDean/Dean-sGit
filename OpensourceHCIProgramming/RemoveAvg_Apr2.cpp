...
void Contraction(int** img, int** img_out, int w) {
	for (int i = 0; i < w - 1; i += 2) {
		for (int j = 0; j < w - 1; j += 2) {
			img_out[i / 2][j / 2] = (img[i][j]
								+ img[i][j + 1]
								+ img[i + 1][j]
								+ img[i + 1][j + 1]) / 4;
		}
...
void main_Mar31() {
	int h, w;
	int** img = ReadImage((char*)"barbara.png", &h, &w);
	int** img_out = IntAlloc2(h, w);

	Contraction(img, img_out, w);
	ImageShow((char*)"barbara_Original", img, h, w);
	ImageShow((char*)"barbara_", img_out, h / 2, w / 2);

	IntFree2(img, h, w);
	IntFree2(img_out, h, w);
}

#define SQ(x) (x*x)
int Average(int** img, int w) {
	int sum=0;
	for (int i = 0; i < w; i++) {
		for (int j = 0; j < w; j++) {
			sum += img[i][j];
		}
	}return int((float)sum / SQ(w) + 0.5);
}

void RemoveAvg(int** img, int** img_out, int w) {
	int m = Average(img, w);
	printf("%d", m);
	for (int y = 0; y < w; y++) {
		for (int x = 0; x < w; x++) {
			img_out[y][x] = img[y][x] - m;
		}
	}
}
...
void main() {
	...
	RemoveAvg(img, img_out, w);

	ImageShow((char*)"barbara_Original", img, h, w);
	ImageShow((char*)"barbara_", img_out, h, w);
